...
public class Ollybolly extends Activity {
	...
	void createThreadAndDialog() {
		...
		pDialog = ProgressDialog.show(Ollybolly.this, "", "loading ", true,
				false);
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				...
				int tableVersionCount = mDBManager
						.getTableCount(DB.T_NAME_VERSION);
				int tableCountryCount = mDBManager
						.getTableCount(DB.T_NAME_COUNTRY);
				...
				try {
					...
					if (tableVersionCount == 0) { // ++버전 테이블 초기 생성시
						...
						if (xmlResult[0].equals(mCommon.xmlVersionList.get(0))
								&& xmlResult[2].equals(mCommon.xmlVersionList
										.get(2))) {// ++pass
							// Log.e("version","pass");
					...
					if (tableCountryCount > 0) {
						...
						String strImage = "data/data/com.ollybolly/files/"
								+ titleImage + mCommon.xmlVersionList.get(6);
					...
					;
					if (tableVersionCount == 0) { // ++버전 xml insert 하기 이미지도 로컬에
													// 다 저장하기
						// ++ 데이터 지우기
						mDBManager.deleteTable(DB.T_NAME_COUNTRY);

						// ++디비에 넣어주기
						Iterator<Integer> iter = inCountryList.keySet()
								.iterator();
						...
						while (iter.hasNext()) {
							 ...
							 * System.out
							 * .println("==nCount=="+nCount+"==i=="+i+"=="
							 * +tmpList.get(i)); }
							 */

							// System.out.println (
							// " value )  "+inList2.get(nCount) ) ;
						...
						while (crsCountry.moveToNext()) {
							mCommon.HttpDown(
									mCommon.xmlVersionList.get(3)
											+ mCommon.xmlVersionList.get(4)
											+ crsCountry.getString(1)
											+ mCommon
													.getReplaceFileName(mCommon.xmlVersionList
															.get(5)),
									crsCountry.getString(1)
											+ mCommon.xmlVersionList.get(5),
									mContext);
							mCommon.HttpDown(
									mCommon.xmlVersionList.get(3)
											+ mCommon.xmlVersionList.get(4)
			...
			case 2:
				...
				mHandler.postDelayed(new Runnable() {
					...
					public void run() {
						...
						Intent intent = new Intent(Ollybolly.this,
								OllybollyMain.class);
