...
public final class XmlParsing {
	...
	public HashMap<Integer, ArrayList<Object>> getXmlCountryResult() {
		 ...
		 * 
		 * 
		 * while(iter.hasNext()) { int nCount = iter.next();
		 * 
			...
			while (parserEvent != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG:
					...
					if (tag.compareTo("media:content") == 0) {
						// 2013.03.25 
						// 속성이 4개였을때만 데이터를 받아오게 처리한 것을 변경함.
						// 17번째컬럼에서 에러발생.
						// java.lang.IndexOutOfBoundsException: Invalid index 17, size is 17
						// parser.getAttributeCount() == 4 -> parser.getAttributeCount() >= 4 
		...
		try {
			...
			while (parserEvent != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.END_TAG:
					...
					/*
					 * isContent[1] = false; isContent[2] = false; isContent[3]
					 * = false; isContent[4] = false; //++url isContent[5] =
					 * false; //++count
					 */
