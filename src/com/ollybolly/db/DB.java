...
public class DB {
	...
	//private final static int DB_MODE = Context.MODE_WORLD_READABLE;
	...
	public static final String COLUMN_MEDIA_CONTENT_BUTTON = "media_content_button";
	...
	public DB(Context context) {
		...
		if (context == null)
			throw new SQLException(
					"fail to open storemanager: context parameter is null");
	...
	public void createTable() {
		...
		try {
			...
			throw new SQLException(
					"fail to T_NAME_VERSION create table: version:"
							+ se.toString());
		} finally {
		...
		try {
			...
			throw new SQLException(
					"fail to T_NAME_COUNTRY create table: country:"
							+ se.toString());
		} finally {
			// Log.e("table create","table create");
		}
		...
		// ++동화 리스트
		// 2013.03.25 - media_content_button column 추가
		sql = "create table if not exists ";
		sql += T_NAME_ITEM;
		...
		sql += COLUMN_MEDIA_CONTENT_BUTTON + " text,";
		...
		try {
			...
			throw new SQLException("fail to T_NAME_ITEM create table: item:"
					+ se.toString());
		} finally {
			// Log.e("table create","table create");
		}
		...
		try {
			...
			throw new SQLException("fail to T_NAME_MYBOX create table: item:"
					+ se.toString());
		} finally {
			// Log.e("table create","table create");
		}
	...
	public void dropTable() {
		...
		try {
			...
			Log.e(DBG_TAG,
					"fail to drop table:" + T_NAME_COUNTRY + se.toString());
			throw new SQLException("fail to drop table:" + T_NAME_COUNTRY
					+ se.toString());
		}
		...
		try {
			...
			throw new SQLException("fail to drop table:" + T_NAME_ITEM
					+ se.toString());
		}
		...
		try {
			...
			throw new SQLException("fail to drop table:" + T_NAME_MYBOX
					+ se.toString());
		}
	...
	public void dropTable(String tableName) {
		...
		try {
			...
			throw new SQLException("fail to drop " + tableName + ":"
					+ se.toString());
		}
	...
	public void deleteTable(String tableName) {
		...
		try {
			...
			throw new SQLException("fail to delete " + tableName + ":"
					+ se.toString());
		}
	...
	public int getTableCount(String TB_NAME) {
		...
		try {
			...
			throw new SQLException("fail to getTableCount list:"
					+ se.toString());
		} catch (Exception se) {
			Log.e(DBG_TAG, "fail to get getTableCount list:" + se.toString());
			throw new SQLException("fail to get getTableCount list:"
					+ se.toString());
		} finally {
			cursor.close();
		}
	...
	// ++version 테이블에 데이터 넣기
	public void insertVersion(String[] result) throws SQLException {

		String sql = "INSERT INTO ";
		try {

			sql += T_NAME_VERSION;
			sql += "(" + COLUMN_VERSION + "," + COLUMN_DATE + ","
					+ COLUMN_CONTENTS_UPDATES + "," + COLUMN_API_DOMAIN + ","
					+ COLUMN_IMG_PATH + "," + COLUMN_IMG_POSTFIX_MAINTITLE
					+ "," + COLUMN_IMG_POSTFIX_MAINFLAG + ",";
			sql += COLUMN_IMG_POSTFIX_TITLE + "," + COLUMN_IMG_POSTFIX_TITLE_EN
					+ "," + COLUMN_IMG_POSTFIX_FLAG + ","
					+ COLUMN_IMG_POSTFIX_BG + "," + COLUMN_IMG_POSTFIX_BG_EN
					+ "," + COLUMN_IMG_POSTFIX_BT + ",";
			sql += COLUMN_IMG_POSTFIX_BT_EN + "," + COLUMN_IMG_POSTFIX_BT_MOV
					+ "," + COLUMN_IMG_POSTFIX_BT_MOV_EN;
			sql += ") VALUES ( '";
			sql += result[0] + "','" + result[1] + "','" + result[2] + "','"
	...
	// ++country 테이블에 데이터 넣기
	public void insertCountry(ArrayList<Object> arrList) throws SQLException {
		// Log.e("tmpList","=="+"'"+ result[0] +"','"+result[1] +"','"+result[2]
		// +"','"+result[3] +"','"+result[4] +"','"+result[5]+"'");
		...
		try {
			...
			if (arrList.size() == 5) {
				...
				sql += T_NAME_COUNTRY;
				sql += "(" + COLUMN_COUNTRY_CODE + "," + COLUMN_COUNTRY_UPDATE
						+ "," + COLUMN_LANGLUAGE_EN + "," + COLUMN_LANGLUAGE_KO
						+ "," + COLUMN_LANGLUAGE_NL;
				sql += ") VALUES ( '";
				sql += arrList.get(0) + "','" + arrList.get(1) + "','"
				...
				sql += "(" + COLUMN_COUNTRY_CODE + "," + COLUMN_COUNTRY_UPDATE
						+ "," + COLUMN_LANGLUAGE_EN + "," + COLUMN_LANGLUAGE_KO
						+ "," + COLUMN_LANGLUAGE_NL + "," + COLUMN_MOVIE_URL;
				sql += ") VALUES ( '";
				sql += arrList.get(0) + "','" + arrList.get(1) + "','"
						+ arrList.get(2) + "','" + arrList.get(3) + "','"
	...
	public void insertItem(int iGuid, ArrayList<Object> arrList)
		...
		try {
			...
			sql += COLUMN_MEDIA_CONTENT_BUTTON + ",";
			...
			sql += ") VALUES ( ";
			sql += iGuid; // guid : 1
			sql += ",'" + getWordReplace((String) arrList.get(0)) + "'"; // title_en : Gold Pit and Silver Pit 1
			sql += ",'" + getWordReplace((String) arrList.get(1)) + "'"; // title_ko : 금구덩이 은구덩이 1
			sql += ",'" + getWordReplace((String) arrList.get(2)) + "'"; // author_en : OllyBolly
			sql += ",'" + getWordReplace((String) arrList.get(3)) + "'"; // author_ko : 올리볼리
			sql += ",'" + getStringDate((String) arrList.get(4)) + "'"; // pub_date : 2010-07-23 15:02:11
			sql += ",'" + getWordReplace((String) arrList.get(5)) + "'"; // description_en : ”This is a story of two brothers who lived in Vietnam. After their parents passed away, the two brothers should have tried harder to get along with each other. But one of them became greedy. Shall we find out what happened to the brothers?”
			sql += ",'" + getWordReplace((String) arrList.get(6)) + "'"; // description_ko : 금구덩이 은구덩이´는 옛날 베트남에서 살았던 어느 형제 이야기에요. 부모님을 일직 여읜 형제는 더 사이좋게 살아야 하지요. 하지만 부자가 된 동생에게 형이 욕심을 부리기 시작해요. 어떤 일이 일어나는지 함게 알아 볼까요?
			sql += ",'" + getWordReplace((String) arrList.get(7)) + "'"; // media_content_url : 
			sql += ",'" + getWordReplace((String) arrList.get(8)) + "'"; // media_content_type : 
			sql += ",'" + getWordReplace((String) arrList.get(9)) + "'"; // media_content_duration : 
			sql += ",'" + getWordReplace((String) arrList.get(10)) + "'"; // media_content_count : 
			sql += ",'" + button + "'"; // media_content_button : com
			sql += ",'" + nation.toLowerCase() + "'"; // media_location_en : Vietnam
			sql += ",'" + getWordReplace((String) arrList.get(12)) + "'"; // media_location_ko : 베트남
			sql += ",'" + getWordReplace((String) arrList.get(13)) + "'"; // media_title : 금구덩이 은구덩이 1
			sql += ",'" + getWordReplace((String) arrList.get(14)) + "'"; // media_credit_owner_en : OllyBolly
			sql += ",'" + getWordReplace((String) arrList.get(15)) + "'"; // media_credit_writer_en : minh quoc
			sql += ",'" + getWordReplace((String) arrList.get(16)) + "'"; // media_credit_illustrator_en : Vu Duy Nghia
			sql += ",'" + getWordReplace((String) arrList.get(17)) + "'"; // media_credit_owner_ko :올리볼리
			sql += ",'" + getWordReplace((String) arrList.get(18)) + "'"; // media_credit_writer_ko : 밍 구억
			sql += ",'" + getWordReplace((String) arrList.get(19)) + "'"; // media_credit_illustrator_ko : 부 쥐 응이아
			sql += ",'" + getWordReplace((String) arrList.get(20)) + "'"; // media_thumbnail : http://apis.ollybolly.org/image/goldpit1.png
	...
	public ArrayList<String> getVersionList() {
		...
		try {
			...
			throw new SQLException("fail to get user version list:"
					+ se.toString());
		} finally {
			cursor.close();
		}
	...
	public String getCountryImage() {
		...
		try {
			...
			throw new SQLException("fail to get user country list:"
					+ se.toString());
		} finally {
			cursor.close();
		}
	...
	public String getCountryImage(String nWhere) {
		...
		try {
			...
			throw new SQLException("fail to get getCountryImage list:"
					+ se.toString());
		} catch (Exception se) {
			Log.e(DBG_TAG, "fail to get MyGymItem list:" + se.toString());
			throw new SQLException("fail to get user getCountryImage list:"
					+ se.toString());
		} finally {
			cursor.close();
		}
	...
	public int getCountryCount() {
		...
		try {
			...
			throw new SQLException("fail to get getCountryCount list:"
					+ se.toString());
		} catch (Exception se) {
			Log.e(DBG_TAG, "fail to get MyGymItem list:" + se.toString());
			throw new SQLException("fail to get user getCountryCount list:"
					+ se.toString());
		} finally {
			cursor.close();
		}
	...
	public Cursor getCountryItem(String nOrder, String nWhere) {
		...
		/*
		 * Cursor test = db.rawQuery(
		 * "SELECT datetime(pubdate,'localtime') FROM  item  where media_location_en = 'mongolia'    order by datetime(pubdate,'localtime') asc, guid asc "
		 * , null); test.moveToFirst();
		 * 
		 * int crsItemCount = test.getCount();
		 * Log.e("crsItemCountdfdffdfdfd",""+crsItemCount); while
		 * (test.moveToNext()) { Log.e("test.getString(3)",test.getString(0)); }
		 */
		...
		String sql = "SELECT " + COLUMN_GUID + "," + COLUMN_PUBDATE + ","
				+ COLUMN_MEDIA_LOCATION_EN + "," + COLUMN_THUMBNAIL + ","
				+ COLUMN_MEDIA_CONTENT_URL + "," + COLUMN_MEDIA_CONTENT_COUNT
				+ "," + COLUMN_DESCRIPTION_EN + "," + COLUMN_DESCRIPTION_KO;
		sql += "," + COLUMN_TITLE_EN + "," + COLUMN_TITLE_KO + ","
				+ COLUMN_CREDIT_WRITER_EN + "," + COLUMN_CREDIT_WRITER_KO + ","
				+ COLUMN_CREDIT_ILLUSTRATOR_EN + ","
				+ COLUMN_CREDIT_ILLUSTRATOR_KO;
		sql += " FROM  ";
	...
	// ++mybox 테이블에 데이터 넣기
	public boolean insertMybox(int guid) throws SQLException {
		boolean isOk = false;
		String sql = "";
		...
		try {
			...
			throw new SQLException("fail to get user getMyboxCount:"
					+ se.toString());
		} finally {
			cursor.close();
		}
	...
	// ++mybox 테이블에 데이터 넣기
	public boolean getGuidMybox(int guid) throws SQLException {
		boolean isOk = false;
		String sql = "";
	...
	public Cursor getMyBoxItem() {
		...
		String tmpOrder = " order by pubdate desc, " + COLUMN_TITLE_KO
				+ " asc ";

		String sql = "SELECT A." + COLUMN_GUID + "," + COLUMN_PUBDATE + ","
				+ COLUMN_MEDIA_LOCATION_EN + "," + COLUMN_THUMBNAIL + ","
				+ COLUMN_MEDIA_CONTENT_URL + "," + COLUMN_MEDIA_CONTENT_COUNT
				+ "," + COLUMN_DESCRIPTION_EN + "," + COLUMN_DESCRIPTION_KO;
		sql += "," + COLUMN_TITLE_EN + "," + COLUMN_TITLE_KO + ","
				...
				+ COLUMN_CREDIT_ILLUSTRATOR_KO;
		sql += " FROM  ";
		sql += T_NAME_ITEM + " A INNER JOIN " + T_NAME_MYBOX
				+ " B ON A.guid = B.guid  " + tmpOrder;
