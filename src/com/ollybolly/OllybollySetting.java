...
public class OllybollySetting extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/**
		 * 1. 설정화면 - 설정의 상단바 하단 위치조정 - “설정” 텍스트 정중앙 위치조정 - 언어 선택의 메뉴 비율조정
		 * @author op
		 */
		...
		if (screenWidth == 1280 && screenHeight == 720) {
			...
			// 설정화면의 상단바
			LinearLayout settings_title_bar = (LinearLayout) findViewById(R.id.settings_title_bar);
			settings_title_bar.setLayoutParams(new LinearLayout.LayoutParams(
					LinearLayout.LayoutParams.FILL_PARENT, 220));

			// 설정화면의 상단아래바
			LinearLayout settings_title_under_bar = (LinearLayout) findViewById(R.id.settings_title_under_bar);
			LinearLayout.MarginLayoutParams lmargin = new LinearLayout.MarginLayoutParams(
					settings_title_under_bar.getLayoutParams());
			lmargin.setMargins(20, 70, 0, 0);
			settings_title_under_bar.setLayoutParams(new LinearLayout.LayoutParams(lmargin));

			// 설정화면의 상단바에 위치한 텍스트
			LinearLayout.MarginLayoutParams margin = new LinearLayout.MarginLayoutParams(
					imgView01.getLayoutParams());
			margin.setMargins(-60, 20, 0, 0);
			imgView01.setLayoutParams(new LinearLayout.LayoutParams(margin));

			// 설정화면의 한국어 선택 버튼
			margin = new ViewGroup.MarginLayoutParams(
					imgView03.getLayoutParams());
			LinearLayout.LayoutParams ko_button_params = new LinearLayout.LayoutParams(
					399, 104);
			ko_button_params.leftMargin = 80;
			imgView03.setLayoutParams(ko_button_params);

			// 설정화면의 영어 선택 버튼
			margin = new ViewGroup.MarginLayoutParams(
					imgView04.getLayoutParams());
			LinearLayout.LayoutParams en_button_params = new LinearLayout.LayoutParams(
					399, 104);
			en_button_params.leftMargin = 30;
			imgView04.setLayoutParams(en_button_params);
			...
			// 설정화면의 언어선택바
			/*
			LinearLayout settings_select_lang_bar = (LinearLayout) findViewById(R.id.settings_select_lang_bar);
			settings_select_lang_bar.setLayoutParams(new LinearLayout.LayoutParams(
					LinearLayout.LayoutParams.FILL_PARENT, 252));
			*/
			// 설정화면의 한국어 선택 버튼
			margin = new ViewGroup.MarginLayoutParams(
					imgView03.getLayoutParams());
		...
		if (Common.langType.equals("en")) {
			...
			imgView04
					.setBackgroundResource(R.drawable.icon_setting_english_off);
		}
		...
		imgView03.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				PreferenceUtil.putPreference(OllybollySetting.this,
						Common.KEY_LANGUAGE, "ko");
				// ++ SharedPreference 셋팅하기 끝

				Intent intent = new Intent(OllybollySetting.this,
	...
	private void clickBack() {
		...
		Intent intent = new Intent(OllybollySetting.this, OllybollyMain.class); // 이것은
																				// 현재페이지에서
																				// 현재페이지
																				// 클래스로..
		intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
